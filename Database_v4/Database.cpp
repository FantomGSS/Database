...
{
    string newCatalogPath = splittedCommand[2];
...
*/
void closeCommand(vector<Entry>& catalog, vector<Entry>& entries)
{
    catalog.clear();
    entries.clear();
...
void commandPattern(string filePath, vector<string>& splittedCommand, vector<Entry>& catalog, vector<Entry>& entries)
{
    ...
    else if (splittedCommand[0] == "save" && splittedCommand[1] == "as" && splittedCommand.size() == 3)
    {
        saveAsCommand(filePath, splittedCommand);
    }
    else if (splittedCommand[0] == "close" && splittedCommand.size() == 1)
    {
        closeCommand(catalog, entries);
    }
